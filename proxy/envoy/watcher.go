	...
	return proxy.Proxy{
		Run: func(config interface{}, epoch int, abort <-chan error) error {
			...
			args := []string{"-c", fname,
				...
				"--drain-time-s", fmt.Sprint(convertDuration(mesh.DrainDuration) / 1 * time.Second),
				"--parent-shutdown-time-s", fmt.Sprint(convertDuration(mesh.ParentShutdownDuration) / 1 * time.Second),
